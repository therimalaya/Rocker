...
RUN R -e "install.packages(c('pls', 'Renvlp', 'ggpubr', 'shinyjs', 'shinydashboard', \
  ...
  && tlmgr install elsarticle subfig enumitem float xcolor colortbl \
  && tlmgr install cleveref tabu varwidth psnfss setspace palatino mathpazo ulem\
  && tlmgr install soul todonotes easyreview ms pgf pdfpages mdframed wallpaper\
  && tlmgr install needspace moderncv fancyhdr microtype fontawesome import \
